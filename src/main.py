...
class GameContext:
    ...
    def __init__(self):
        ...
        self.display = is31.Matrix(i2c)
        self.display.fill(0)

        # 按键
        self.key = Pin(9, Pin.IN, Pin.PULL_UP)
        self._button_raw = self.key.value()        # 原始读取值（瞬时）
        self._button_stable = self._button_raw     # 已稳定的值（经过防抖）
        self._button_prev_stable = self._button_raw
        self._button_last_change = time.ticks_ms()
        self._debounce_ms = 50                     # 防抖时间：50ms
    ...
    def debounce_key(self):
        """
        轮询按键，只有当检测到“稳定的按下(0) -> 稳定的抬起(1)”边沿时返回 True。
        调用者应在主循环中频繁调用（每帧/每次迭代）。
        """
        now = time.ticks_ms()
        raw = self.key.value() 
